...
public:
	...
	virtual void mvmult_cmplx(const vecc& vec_in, vecc& vec_out) = 0;
...
public:
	...
	void mvmult_cmplx(const vecc& vec_in, vecc& vec_out) {
		if (vec_in.size() != this->size or vec_out.size() != this->size) 
			std::cout << "MVMULT matrix size mismatch!" << std::endl;
...
private:
	...
	void sparse_mvmult(const U* vec_in, U* vec_out) {
	    // Zero-initialize the output vector
	    std::fill(vec_out, vec_out + this->size, U(0));
	    int nthreads = omp_get_max_threads();
	    std::vector<std::vector<U>> thread_bufs(nthreads, std::vector<U>(this->size, U(0)));
		#pragma omp parallel
	    {
	        int tid = omp_get_thread_num();
	        auto& local_out = thread_bufs[tid];
	        for (size_t e = 0; e < val.size(); ++e) {
	            auto i = indexi[e];
	            auto j = indexj[e];
	            auto v = val[e];
	            ...
	            if (this->mat_type == "S") {
	                // If the matrix is symmetric, add both off-diagonal elements
	                local_out[j] += v * vec_in[i];
	                if (i != j)
	                    local_out[i] += v * vec_in[j];
	            } else {
	                // If the matrix is not symmetric, only add the off-diagonal element
	                local_out[j] += v * vec_in[i];
	            }
	        }
	    }
	    // Final accumulation of thread-local buffers into the output vector
	    #pragma omp parallel for
	    for (int i = 0; i < this->size; ++i) {
	        for (int t = 0; t < nthreads; ++t)
	            vec_out[i] += thread_bufs[t][i];
	    }
	}     
};
